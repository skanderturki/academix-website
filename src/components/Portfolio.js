...
function WorkflowCard({ workflow, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group text-left h-full rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-6 transition hover:border-gold/40 hover:bg-white/[0.06] hover:-translate-y-1"
    >
      <div className="flex items-start gap-4 mb-4">
        <div className="flex-shrink-0 text-4xl">{workflow.icon}</div>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap gap-2 mb-2">
            <Badge className="border-brand-navy/40 bg-brand-navy/15 text-gold">
              {workflow.category}
            </Badge>
      ...
      <div className="flex items-center gap-2 text-sm font-medium text-gold group-hover:text-gold-light">
        View details
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {workflow.nodes.map((node, idx) => (
                    <div key={node} className="flex items-center gap-2">
                      <span className="inline-flex items-center rounded-lg border border-gold/30 bg-gold/10 px-3 py-1.5 text-xs font-medium text-gold">
                        {node}
                      </span>
            ...
            <>
              ...
              <div className="rounded-lg border border-gold/30 bg-gold/10 p-4">
                <p className="text-sm text-white/80">
                  <strong className="text-gold">Need Help?</strong> Our team can help you implement this workflow or customize it for your specific needs.{' '}
                  <a
                    href="#contact"
    ...
    <section className="relative bg-mesh-dark min-h-screen">
      ...
      <div className="container-page relative py-16 md:py-20">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-medium text-gold uppercase tracking-widest mb-6">
            Portfolio
          </div>
